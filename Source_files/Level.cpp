...
Level::Level(int WindowWidth, int WindowHeight, int n, int l)
    : WindowWidth(WindowWidth), WindowHeight(WindowHeight), n(n), l(l) {
    map = new Map();
    if (n != 4) {
        readMap(map, n);
        map->ColorMap();
    ...
    while (!WindowShouldClose()) {
        ...
        if (l != 3) {
            for (int i = 0; i < 4; i++) {
                ...
                if (relativePos.y >= 1)
                    ghost->goUp(i); // it's on top of it
                else if (relativePos.y <= -1)
                    ghost->goDown(i);
                if (relativePos.x >= 1)
                    ghost->goLeft(i); // it's in the same column, let's see the difference in rows
        ...
        if (mapdrawn == false && n == 4) {
            ...
            mapdrawn = true;
            for (int i = 0; i < 4; i++)
                ghost->InitialPosition[i] = map->getClPos(GetRandomCl(map));
            // writeMap(map,3);
            food->update(map);
        }
